...
for (k in 1:K_final) {
  topic_beta <- beta_mat[k,] %>% sort(decreasing = TRUE)
  
  wordcloud(
    words= stm_model$vocab %>% strsplit("/") %>% sapply(\(x) x[1]),
    freq = topic_beta,
    max.words=50,
    scale=c(4,0.8),
    random.order=FALSE,
  ...
  title(paste("Topic",k,"Worldcloud"))
}
```

## 7.2. TF-IDF-based Wordclouds

```{r}
dtm_tfidf_mat <- dtm

top_words <- labelTopics(stm_model, n = ncol(dtm_tfidf_matrix))
...
for (k in 1:K_final) {
  ...
  topic_beta_sorted <- sort(topic_beta, decreasing = TRUE)
  
  wordcloud(
    words= stm_model$vocab %>% strsplit("/") %>% sapply(\(x) x[1]),
    freq = topic_beta_sorted,
    max.words=50,
    scale=c(4,0.8),
    random.order=FALSE,
  )
