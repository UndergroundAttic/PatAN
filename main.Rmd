...
make.dt(stm_model, meta=corpus$meta) %>% 
    ...
    cor() %>% 
    pheatmap(display_numbers=T, cutree_rows=k_val, cutree_cols=k_val)
```
...
normalized_cor <- scale(stm_cor$cor)

stm_hc <- hclust(normalized_cor %>% dist(method="euclidean"), "ward.D2")
...
normalized_cor %>% kmeans(k_val) %>% fviz_cluster(normalized_cor)
